...
export function NightBatchPanel({ fps, assets, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      ...
      // 均等: 投入数が最小 → 同数ならカット番号順
      const next = [...targets].sort((a, b) =>
        (queuedCount[a.n] ?? 0) - (queuedCount[b.n] ?? 0) || a.n - b.n)[0]
      ...
      try {
        ...
        setMsg(`🌙 C${next.n} をキュー投入(累計 ${Object.values(queuedCount).reduce((a, b) => a + b, 0) + 1}本)`)
      } catch {
  ...
  }, [running, cuts, selected, queuedCount])
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/70 p-2 sm:p-6" onClick={onClose}>
      <div onClick={e => e.stopPropagation()}
        ...
        <div className="flex items-center gap-2 flex-wrap text-[10px]">
          <button onClick={() => setSelected(new Set(eligible.map(c => c.n)))}
                  className="px-2 py-1 rounded bg-zinc-800 text-zinc-300 hover:bg-zinc-700">全選択({eligible.length})</button>
          <button onClick={() => setSelected(new Set())}
                  className="px-2 py-1 rounded bg-zinc-800 text-zinc-400 hover:bg-zinc-700">クリア</button>
          <span className="text-zinc-500">選択 {selected.size}件 / 実行中・待機 {activeJobs}件</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-1 max-h-[52vh] overflow-y-auto pr-1">
          {cuts.map(c => {
            const on = selected.has(c.n)
            const disabled = c.locked || !c.hasParams
            return (
              <button key={c.n} onClick={() => !disabled && toggle(c.n)} disabled={disabled}
                      className={`text-left text-[10px] px-2 py-1.5 rounded border flex flex-col gap-0.5
                        ${disabled ? 'border-zinc-800 bg-zinc-950 text-zinc-600'
                          : on ? 'border-purple-400 bg-purple-950/40 text-purple-100'
                               : 'border-zinc-700 bg-zinc-800/60 text-zinc-300 hover:border-zinc-500'}`}
                      title={c.locked ? '🔒ロック中のため対象外' : !c.hasParams ? '生成履歴がないため対象外' : ''}>
