...
タイポグラフィを画面に**載せる装飾**ではなく、画を**切り抜くマスク**として使う。
文字の内側にキャラクターの断片が別々に覗く。

本作への翻訳: C24「DISTANCE POINT」は文字を闇に浮かべるだけだった。
文字の中にほむらとまどかの断片を別々に入れれば、**文字自体が二人を隔てる壁**になる。
歌詞を画にするという方針とも噛み合う。
...
余計な文字・字幕・透かしの混入を防ぐ。日本語グリフが使えることは実測済み(job 3500)なので相性が良い。
