...
## プロジェクトのエクスポート/インポート(2026-07-03 実装)

- ヘッダー「📦 書き出し」→ `.kycha.zip`(サイドバー📥で復元)。API: `POST /api/projects/{id}/export`, `POST /api/projects/import`
- アーカイブは自己完結(アセット実ファイル・コメント・mad-kit一式同梱。サムネ/プロキシ/QA/最終mp4は除外)
- **後方互換の設計**(services/project_archive.py):
  1. `manifest.format_version` + `MIGRATIONS`辞書 — スキーマ変更時はバージョン+1と移行関数1つの追加で古いアーカイブを読み続けられる
  2. 寛容リーダー — 未知フィールド無視・欠損はモデルデフォルト・ISO日時復元
  3. ID非依存 — 参照は配列index/相対パス/shot_id文字列。インポートでID全振り直し
- 終了プロジェクトの退避運用: 書き出し→zipを別ストレージへ→アプリからプロジェクト削除、で本体を軽く保てる
