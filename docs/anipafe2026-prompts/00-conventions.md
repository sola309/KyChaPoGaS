...
| タグ | 展開元 |
|---|---|
| `{{HOMU_ID}}` | homura_devil + 「スカートは**床まで届き前が開いている**・片脚は菱形市松ストッキング/もう片脚は素足に赤リボン螺旋・指先まで手甲」の固定追記 |
...
`{{S:A3}}`幕3 / `{{S:WATER}}`水面 / `{{FOCUS}}` / `{{STYLE}}`(セル画+NO-TEXT+資料枠禁止) / `{{LENS}}` / `{{DEPTH}}`(深度面5-8枚の速度差=視差)
...
| 単位 | カット | 話者 | 言語 | 性質 |
|---|---|---|---|---|
| U13 | C14 | アルティメットまどか | **日本語** | 神が誘う |
| U14 | C15 | 悪魔ほむら(反射の口元のみ) | **日本語** | 人間が拒む＝引き裂き |
| U29 | C45 | 悪魔ほむら | **英語** | 神が告げる |
| U30 | C46 | 人間まどか | **英語** | 人間が拒む |

**過去の対は日本語、現在の対は英語**(歌詞そのものがそうなっている)。
同じ役割で立ち位置だけが入れ替わり、拒絶の尺も逆転する(C15=2.33s短く突き放す /
C46=6.96s長く涙ぐみながら)。

⚠ **キュゥべえ(C33/C34/C65/C66)はリップシンクしない** — テレパシーで話すため口が動かない(公式設定)。
声だけが降り、その効果で存在を示す。憲法32条。
