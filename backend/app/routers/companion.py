...
- 文が2つあるときは、それぞれの文の直後に |感情 を付けてよい（表情が文ごとに変わる）。
例:
へえ、いい度胸じゃん|smile
は？|surprised あんた本気で言ってんの？|angry
へえ、いい度胸じゃん|smile
...
def _parse_segments(text: str) -> list[dict]:
    """'文A|expr 文B|expr' 形式を [(text, expr)...] に分解(タグ無し文はneutral継承)。"""
    segs = []
    for part in re.split(r"(?<=[|｜][a-z]{3,9})\s+|\n", text.strip()):
        part = part.strip()
        if not part:
            continue
        mm = re.search(r"[|｜]\s*([A-Za-z぀-ヿ一-鿿]+)\s*$", part)
        expr = "neutral"
        if mm:
            expr = _EXPR_MAP.get(mm.group(1).lower(), "neutral")
            part = part[:mm.start()].strip()
        if part:
            segs.append({"text": part, "expression": expr})
    return segs
...
class ChatResp(BaseModel):
    ...
    expression: str
    provider: str
    segments: list[dict] = []   # [{text, expression}] 文ごとの感情タイムライン
...
def chat(req: ChatReq):
    ...
    return ChatResp(reply=reply or "……", expression=expression, provider=prov,
                    segments=_parse_segments(text) or [{"text": reply, "expression": expression}])
